...
def main(n, xyr):
    ...
    while stopper(time_limit):
        ...
        # 長く経過するとTが減るように適当に重み付け
        T = pow(t0, 1.0 - t) * pow(t1, t)
        # 改善するか、一定確率にヒットしたら更新する
        def scheduler(delta):
...
def visualizer(n, xyr, ans, p):
    ...
    for abcd in ans:
        draw.rectangle(
            abcd, fill=(randrange(32, 224), randrange(32, 224), randrange(32, 224))
        )
    ...
    for i in range(n):
        ...
        d = min(size, y + r)
        draw.ellipse(
            (a, b, c, d), fill=(int(p[i] * 255), int(p[i] * 255), int(p[i] * 255))
        )

        a, b, c, d = ans[i]
        draw.line(
            ((x, y), ((a + c) // 2, (b + d) // 2)), fill=(255, 255, 255), width=10
        )
